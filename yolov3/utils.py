...
def Load_Yolo_model():
    ...
    if YOLO_FRAMEWORK == "tf":  # TensorFlow detection
        ...
        # logging.debug(saved_model_loaded)
        signature_keys = list(saved_model_loaded.signatures.keys())
        # print(signature_keys)
        # logging.debug(signature_keys)
...
def detect_image(
    ...
    bboxes = nms(bboxes, iou_threshold, method="nms")  # 丢多余的的壳子

    image = draw_bbox(
        original_image, bboxes, CLASSES=CLASSES, rectangle_colors=rectangle_colors
    )
    ...
    return original_image, bboxes, CLASSES
...
def postprocess_mp(
    ...
    while True:
        if Predicted_data.qsize() > 0:
            ...
            bboxes = nms(bboxes, iou_threshold, method="nms")  ####out put in the box
            image = draw_bbox(
                original_image,
                bboxes,
                CLASSES=CLASSES,
...
def detect_video_realtime_mp(
    ...
    p1 = Process(
        target=Predict_bbox_mp, args=(Frames_data, Predicted_data, Processing_times)
    )
    p2 = Process(
    ...
    while True:
        if (
            original_frames.qsize() == 0
            and Frames_data.qsize() == 0
            and Predicted_data.qsize() == 0
            and Processed_frames.qsize() == 0
            and Processing_times.qsize() == 0
            and Final_frames.qsize() == 0
        ):
            p1.terminate()
            p2.terminate()
            p3.terminate()
...
def detect_realtime(
    ...
    times = []
    source = "rtsp://admin:smartCare108@192.168.12.222:554/live"
    ...
    while True:
        ...
        frame = draw_bbox(
            original_frame, bboxes, CLASSES=CLASSES, rectangle_colors=rectangle_colors
        )
